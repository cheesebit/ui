 ...
 *
 * @param {string | number} number - Number to be validated.
...
export function validateMinValue( number, minValue ) {
	number = Number.parseFloat( String( number ) );
...
export function validateRange( number, minValue, maxValue ) {
	return (
		validateMinValue( number, minValue ) &&
		validateMaxValue( number, maxValue )
	);
}
