...
describe( 'validators.number.value', () => {
	describe( 'Validate minimum value', () => {
		it( 'returns true', () => {
			expect(
				validateMinValue( generator.natural( { min: 2 } ), 2 )
			).toBe( true );
		} );
		it( 'returns false', () => {
			...
			expect(
				validateMinValue( generator.natural( { max: 9 } ), 10 )
			).toBe( false );
		} );
	} );
	...
	describe( 'Validate maximum value', () => {
		it( 'returns true', () => {
			expect(
				validateMaxValue( generator.natural( { max: 10 } ), 10 )
			).toBe( true );
		} );
		it( 'returns false', () => {
			...
			expect(
				validateMaxValue( generator.natural( { min: 10 } ), 9 )
			).toBe( false );
		} );
	} );
	...
	describe( 'Validate range value', () => {
		...
		it( 'returns false', () => {
			...
			expect(
				validateRange( generator.natural( { max: 0 } ), 1, 3 )
			).toBe( false );
			expect(
				validateRange( generator.natural( { min: 4 } ), 1, 4 )
			).toBe( false );
		} );
	} );
} );
