...
describe( 'useFingerprint', () => {
	...
	it( 'checks the fingerprint of a set of items against the current fingerprint, regardless order', () => {
		...
		expect( result.current.hasSameFingerprint( [ 'D', 'E', 'F' ] ) ).toBe(
			false
		);
		expect( result.current.hasSameFingerprint( [ 'A', 'B', 'C' ] ) ).toBe(
			true
		);
		expect( result.current.hasSameFingerprint( [ 'A', 'C', 'B' ] ) ).toBe(
			true
		);
		expect( result.current.hasSameFingerprint( [ 'B', 'A', 'C' ] ) ).toBe(
			true
		);
		expect( result.current.hasSameFingerprint( [ 'B', 'C', 'A' ] ) ).toBe(
			true
		);
		expect( result.current.hasSameFingerprint( [ 'C', 'A', 'B' ] ) ).toBe(
			true
		);
		expect( result.current.hasSameFingerprint( [ 'C', 'B', 'A' ] ) ).toBe(
			true
		);
	} );
